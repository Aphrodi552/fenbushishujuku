...
public class AdminUserServiceImpl implements AdminUserService {
    ...
    public Page<AdminUserVO> pageUsers(AdminUserQueryRequest req) {
        ...
        // 按创建时间倒序（如果 createdAt 为空，也不影响编译；只是排序效果可能不理想）
    ...
    public AdminUserVO createUser(AdminUserCreateRequest req) {
        ...
        // password 默认 DEFAULT_PASSWORD
        String pwd = StringUtils.hasText(req.getUserPassword()) ? req.getUserPassword() : DEFAULT_PASSWORD;

        // 注意：你现有库里 password 可能是 BCrypt（$2a$10...），这里为避免引入依赖导致编译失败，先直接存明文。
        // 如果你确认项目用了 BCrypt，请把这里改为 BCrypt 加密后再存（我可以再给你对应版本）。
    ...
    public void resetPassword(String userId) {
        ...
        // 同 createUser：为避免额外依赖，这里直接写明文默认密码
